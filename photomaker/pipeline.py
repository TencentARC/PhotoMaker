...
class PhotoMakerStableDiffusionXLPipeline(StableDiffusionXLPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # 13. Create down mid and up layer lists
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #pag
                    ...
                    if i <= start_merge_step:
                        current_prompt_embeds = torch.cat([prompt_embeds_text_only, prompt_embeds_text_only], dim=0)
                        add_text_embeds = torch.cat([pooled_prompt_embeds_text_only, pooled_prompt_embeds_text_only], dim=0)
                    else:
                        current_prompt_embeds = torch.cat([prompt_embeds, prompt_embeds], dim=0)
                        add_text_embeds = torch.cat([pooled_prompt_embeds, pooled_prompt_embeds], dim=0)
                #both
                elif self.do_classifier_free_guidance and self.do_adversarial_guidance:
                    if i <= start_merge_step:
                        current_prompt_embeds = torch.cat([negative_prompt_embeds, prompt_embeds_text_only, prompt_embeds_text_only], dim=0)
                        add_text_embeds = torch.cat([negative_pooled_prompt_embeds, pooled_prompt_embeds_text_only, pooled_prompt_embeds_text_only], dim=0)
                    else:
                        current_prompt_embeds = torch.cat([negative_prompt_embeds, prompt_embeds, prompt_embeds], dim=0)
                        add_text_embeds = torch.cat([negative_pooled_prompt_embeds, pooled_prompt_embeds, pooled_prompt_embeds], dim=0)
                #nothing
                else:
                    if i <= start_merge_step:
